...
public:
  ...
  void    procArgs(TCchar* m_lpCmdLine);    // Typically called from InitInstance of a Windows
                                            // Application m_lpCmdLine is a member of CWinApp
...
// Use the following three functions from the calling process to start another process passing
// arguments.  Arguments will be enclosed in quotes by the start function.  The first argument must
// be the name or path of the executable that is to be started. Here is an example:
//
//   Executable exe;  exe.start(3, _T("myApp.exe"), _T("Arg1"), _T("Arg2"));

  bool    start(int nCmdArgs, ...);             // nCmdArgs indicates the number of optional
                                                // arguments.  All the optional arguments must be
                                                // of type TCchar*.
