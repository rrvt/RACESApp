// RacesNewDlg.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(RacesNewDlg, CDialogEx)



RacesNewDlg::RacesNewDlg(TCchar* helpPth, CWnd* pParent) : CDialogEx(IDD_RacesNew, pParent),
...
RacesNewDlg::~RacesNewDlg() {winPos.~WinPos();}                             //deleteCtl.CleanUp();




BEGIN_MESSAGE_MAP(RacesNewDlg, CDialogEx)
...
void RacesNewDlg::DoDataExchange(CDataExchange* pDX) {
...
int RacesNewDlg::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
CRect winRect;
  ...
  mbrStatus.load();   mbrAvailability.load();   mbrGeography.load();
...
void RacesNewDlg::onEditRecords() {false;   setStatus(dlgSource, false);}


void RacesNewDlg::onLoadDatabase() {
PathDlgDsc dsc(_T("Database"), 0, _T("accdb"), _T("*.accdb"));
  ...
  iniFile.writeString(GlobalSect, DBFileKey, path);   loadDatabase();
  }


void RacesNewDlg::loadDatabase() {
...
//  mbrRcd = 0;   mbrEnt = 0;   iceEnt = 0;   emplEnt = 0;
...
void RacesNewDlg::loadMbrs(DlgSource src) {
...
void RacesNewDlg::onFindICEAddr() {
AddressDlg dlg;
...
void RacesNewDlg::onFindEmplAddr() {
AddressDlg dlg;
...
void RacesNewDlg::onFindEmplZip() {
CityStateDlg dlg;
...
void RacesNewDlg::onClearMbrfirstname()     {clrLbl(firstNameCtl,      FirstNameLbl);}
void RacesNewDlg::onClearMbrMiddleInitial() {clrLbl(midInitialCtl,     MiddleInitialLbl);}
void RacesNewDlg::onClearMbrLastName()      {clrLbl(lastNameCtl,       LastNameLbl);}
void RacesNewDlg::onClearSuffix()           {clrLbl(suffixCtl,         SuffixLbl);}
void RacesNewDlg::onClearMbrCallsign()      {clrLbl(callSignCtl,       CallLbl);}
void RacesNewDlg::onClearCallSignExpDate()  {clrLbl(csExpDateCtl,      CSExpDateLbl);}
void RacesNewDlg::onClearBadgeExpDate()     {clrLbl(badgeExpDateCtl,   BgExpDateLbl);}
void RacesNewDlg::onClearMbrStreetAdr()     {clrLbl(mbrStreetAdrCtl,   StreetAddrLbl);}
void RacesNewDlg::onClearMbrUnitNo()        {clrLbl(mbrUnitNoCtl,      UnitNoLbl);}
void RacesNewDlg::onClearMbrCity()          {clrLbl(mbrCityCtl,        CityLbl);}
void RacesNewDlg::onClearMbrState()         {clrLbl(mbrStateCtl,       StateLbl);}
void RacesNewDlg::onClearMbrZip()           {clrLbl(mbrZipCtl,         ZipLbl);}
void RacesNewDlg::onClearMbrCellPh()        {clrLbl(mbrCellPhCtl,      CellPhLbl);}
void RacesNewDlg::onClearMbrLandline()      {clrLbl(mbrLandlineCtl,    LandLineLbl);}
void RacesNewDlg::onClearMbrHomeZip()       {clrLbl(mbrHomeZipCtl,     HomeZipLbl);}
void RacesNewDlg::onClearMbrEmail()         {clrLbl(mbrEmailCtl,       EmailLbl);}
void RacesNewDlg::onClearStartDate()        {clrLbl(startDateCtl,      DateLbl);}
void RacesNewDlg::onClearDSWDate()          {clrLbl(dswDateCtl,        DateLbl);}
void RacesNewDlg::onClearResponderDate()    {clrLbl(responderDateCtl,  DateLbl);}

void RacesNewDlg::onClearIceFirstName()     {clrLbl(iceFirstNameCtl,   FirstNameLbl);}
void RacesNewDlg::onClearIceMidInitial()    {clrLbl(iceMidInitialCtl,  MiddleInitialLbl);}
void RacesNewDlg::onClearIceLastName()      {clrLbl(iceLastNameCtl,    LastNameLbl);}
void RacesNewDlg::onClearIceEmail()         {clrLbl(iceEmailCtl,       EmailLbl);}
void RacesNewDlg::onClearIceStreetAdr()     {clrLbl(iceStreetAdrCtl,   StreetAddrLbl);}
void RacesNewDlg::onClearIceUnitNo()        {clrLbl(iceUnitNoCtl,      UnitNoLbl);}
void RacesNewDlg::onClearIceCity()          {clrLbl(iceCityCtl,        CityLbl);}
void RacesNewDlg::onClearIceState()         {clrLbl(iceStateCtl,       StateLbl);}
void RacesNewDlg::onClearIceZip()           {clrLbl(iceZipCtl,         ZipLbl);}
void RacesNewDlg::onClearIceCellPh()        {clrLbl(iceCellPhCtl,      CellPhLbl);}
void RacesNewDlg::onClearIceLandline()      {clrLbl(iceLandlineCtl,    LandLineLbl);}

void RacesNewDlg::onClearEmplName()         {clrLbl(emplNameCtl,       CompanyNameLbl);}
void RacesNewDlg::onClearEmplEmail()        {clrLbl(emplEmailCtl,      EmailLbl);}
void RacesNewDlg::onClearEmplStreetAdr()    {clrLbl(emplStreetAdrCtl,  StreetAddrLbl);}
void RacesNewDlg::onClearEmplUnitNo()       {clrLbl(emplUnitNoCtl,     UnitNoLbl);}
void RacesNewDlg::onClearEmplCity()         {clrLbl(emplCityCtl,       CityLbl);}
void RacesNewDlg::onClearEmplState()        {clrLbl(emplStateCtl,      StateLbl);}
void RacesNewDlg::onClearEmplZip()          {clrLbl(emplZipCtl,        ZipLbl);}
void RacesNewDlg::onClearEmplCellPh()       {clrLbl(emplCellPhCtl,     CellPhLbl);}
void RacesNewDlg::onClearEmplLandline()     {clrLbl(emplLandlineCtl,   LandLineLbl);}
void RacesNewDlg::onClearEmplCompanyZip()   {clrLbl(emplCompanyZipCtl, CompanyZipLbl);}



void RacesNewDlg::onCheckList() {
ChkListDlg dlg;
String     sect;
...
bool RacesNewDlg::setEntity(EntRcd* ent, AdrRcd*& adr, CtyRcd*& cty) {
int id;
...
void RacesNewDlg::saveNewMember() {
Cstring         cs;
MbrRcd          rcd;
String          s;
...
int RacesNewDlg::addEntity(EntRcd& ent) {
EntRcd* entRcd;
...
void RacesNewDlg::onLeaveIceZip() {
...
void RacesNewDlg::onLeaveEmplZip() {
...
void RacesNewDlg::onLeaveIceName() {
PrepAdrRcd prepRcd;
...
bool RacesNewDlg::setField(String& fld, TCchar* tc)
                                      {if (fld == tc)   return false;   fld = tc;     return true;}
bool RacesNewDlg::setField(int&   fld, int v)
                                      {if (v == fld)    return false;   fld = v;      return true;}


String& RacesNewDlg::getToday() {
static String s;
Date          today;   today.getToday();   return s = today.format(_T("%m%d%y"));
  }


int RacesNewDlg::getNextBadgeNo() {
MbrIter iter(mbrTbl);
MbrRcd* rcd;
int     badgeNo;
...
String RacesNewDlg::expandDate(TCchar* tc) {
String dt = tc;
String s;
...
String RacesNewDlg::compressDate(TCchar* cs) {
String dt = cs;
int    pos;
int    pos2;
String frag;
...
String RacesNewDlg::expandPhone(TCchar* tc) {
String s   = tc;      s.trim();
int    lng = s.length();
String t;
...
String RacesNewDlg::compressPhone(TCchar* tc) {
String s = tc;      s.trim();
int    lng = s.length();
int    pos = 0;
int    pos2;
...
String RacesNewDlg::adjFrag(String& frag) {
int lng;
...
void RacesNewDlg::onUpdateDbExit() {
...
void RacesNewDlg::setStatus(DlgSource src, bool rdOnly) {
String s;
...
void RacesNewDlg::setPath(TCchar* path) {statusBar.setText(1, path);}


void RacesNewDlg::OnMove(int x, int y)
      {CRect winRect;   GetWindowRect(&winRect);   winPos.set(winRect);   CDialogEx::OnMove(x, y);}


void RacesNewDlg::OnSize(UINT nType, int cx, int cy) {
CRect r;
...
void RacesNewDlg::chkMbrPicture() {
CRect r;
int   width;
int   height;
...
LRESULT RacesNewDlg::OnResetToolBar(WPARAM wParam, LPARAM lParam) {setupToolBar();  return 0;}


void RacesNewDlg::setupToolBar() { }


// Do ToolTips

BOOL RacesNewDlg::OnTtnNeedText(UINT id, NMHDR* pNMHDR, LRESULT* pResult)
                                                            {return toolBar.OnTtnNeedText(pNMHDR);}


void RacesNewDlg::onHelp() {
...
void RacesNewDlg::onAbout() {AboutDlg aboutDlg; aboutDlg.DoModal();}
...
#if 0
bool RacesNewDlg::setPtrs(MbrInfo* info) {
